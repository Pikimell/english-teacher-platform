...
}`.trim(),user:f(e,"context",r)}},i=a[t]||a.mcq,l=(n.additionalInstructions||n.extraInstructions||"").trim();return l?{system:i.system,user:`${i.user}

Additional teacher instructions:
${l}`}:i}const Ze={mcq:10,gap:10,transform:10,match:8,error:10,order:10,short:3,writing:1,roleplay:8,"dialogue-gap":6,"dialogue-order":6,truefalse:8,"definition-match":8,"synonym-clue":8,scramble:8,wordpairs:8,"odd-one-out":8,context:4};function f(e,t,n){return`Topic: ${e}
...
Target vocabulary:
${r}`:n||e||""}function nt(e){if(!Array.isArray(e)||!e.length)return"";const t=r=>{if(!r)return"";const o=r.indexOf(" — "),s=r.indexOf(" – "),a=r.indexOf(" - "),i=[o,s,a].filter(y=>y>=0),l=i.length?Math.min(...i):-1;return(l>=0?r.slice(0,l):r).trim()},n=e.map((r,o)=>{if(!r)return"";const s=r.word||r.term||r.phrase||r.text||`Item ${o+1}`,a=r.translation||r.meaning||r.ua||r.uk||"",i=r.example||r.sentence||r.usage||r.sample||"",l=t(i);let c=`${o+1}. ${s}`;return a&&(c+=` — ${a}`),l&&(c+=` (Example: ${l})`),c}).filter(Boolean);return n.length?n.join(`
`):""}const be=document.querySelector(".js-practice-section"),m=document.querySelector("[data-practice-generator]"),Y=m==null?void 0:m.querySelector("[data-practice-status]"),Q=document.querySelector("[data-generator-output]"),B=document.querySelector("[data-generator-json]"),_=document.querySelector("[data-generator-copy]"),P=document.querySelector("[data-generator-download]"),W=m==null?void 0:m.querySelector("[data-types]"),S=m==null?void 0:m.querySelector("[data-types-trigger]"),M=m==null?void 0:m.querySelector("[data-types-panel]"),xe=document.querySelector("[data-practice-placeholder]"),E=window.lessonContext||{};let C=E.title||"Generated Practice",I=E.level||"custom";const N=[],K=[];function O(e){e&&e.classList.add("hidden")}function ie(e){e&&e.classList.remove("hidden")}function Ie(e){return!e||e.classList.contains("hidden")}function le(){return localStorage.getItem("hideGenerateSection")==="true"}function te(){le()?(O(be),O(M),O(Q),U()):(ie(be),re(),q())}function ne(e){return`${e&&typeof e=="object"&&e.id?String(e.id):String((e==null?void 0:e.type)||"task")}-${Date.now()}-${Math.random().toString(16).slice(2,8)}`}function Me(){return{title:C||E.title||"Generated Practice",level:I||E.level||"custom",tasks:[...N,...K].map(e=>e.data)}}function q(){if(B){if(le()){O(Q);return}if(!N.length&&!K.length){B.textContent="",O(Q),_&&(_.disabled=!0),P&&(P.disabled=!0);return}B.textContent=JSON.stringify(Me(),null,2),ie(Q),_&&(_.disabled=!1),P&&(P.disabled=!1)}}function rt(e){const t=ne(e);return K.push({key:t,data:e}),q(),t}function ot(e,t){const n=o=>{const s=o.findIndex(a=>e&&a.key===e||a.data===t);return s!==-1?(o.splice(s,1),!0):!1};(n(K)||n(N))&&q()}function at(){if(!m)return;const e=m.querySelector('[name="topic"]');if(e&&!e.value){const t=E.title||"Lesson topic";e.value=t}}function x(e,t="idle"){Y&&(Y.textContent=e,Y.dataset.state=t)}function st(e){return String(e||"").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"").slice(0,8)}function V(e){return String(e||"").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"")}function ce(){return Array.from(m.querySelectorAll('input[name="types"]'))}function it(e){const t=e.filter(n=>n.checked).map(n=>{var r,o;return(o=(r=n.nextElementSibling)==null?void 0:r.textContent)==null?void 0:o.trim()}).filter(Boolean);return t.length?t.length<=2?t.join(", "):`${t.slice(0,2).join(", ")} +${t.length-2}`:"Оберіть типи"}function re(){if(!S)return;const e=ce();S.textContent=it(e)}function U(){!M||!S||Ie(M)||(O(M),S.setAttribute("aria-expanded","false"))}function lt(){if(!M||!S)return;if(Ie(M)){ie(M),S.setAttribute("aria-expanded","true");const t=M.querySelector('input[name="types"]');t==null||t.focus({preventScroll:!0})}else U()}function ct(){const e=!le();localStorage.setItem("hideGenerateSection",String(e)),te(),q()}function dt(){const e=document.getElementById("practice"),t=e==null?void 0:e.querySelector("#practice-body");if(xe&&xe.remove(),t&&t.children.length===1){const n=t.firstElementChild;n&&/практика поки відсутня/i.test(n.textContent||"")&&n.remove()}}function ut(){const e=document.getElementById("practice");if(!e)return;const t=e.querySelector("#practice-body");if(t&&!t.querySelector(".practice-dynamic-title")){const n=document.createElement("div");n.className="practice-dynamic-title",n.innerHTML=`
      <h2>Згенеровані завдання</h2>
      ${E.level?`<p class="muted">Рівень: ${E.level}</p>`:""}
    `,t.prepend(n)}}async function mt(e){var L,$;if(e.preventDefault(),!m)return;const t=m.querySelector('button[type="submit"]'),n=m.querySelector('[name="topic"]'),r=m.querySelector('[name="count"]'),o=m.querySelector('[name="instructions"]'),s=ce(),a=(L=n==null?void 0:n.value)==null?void 0:L.trim(),i=Number.parseInt((r==null?void 0:r.value)||"10",10)||10,l=(($=o==null?void 0:o.value)==null?void 0:$.trim())||"",c=s.filter(u=>u.checked).map(u=>u.value);if(!a){x("Укажіть тему для генерації завдання.","error"),n==null||n.focus();return}if(!c.length&&typeof U=="function"){x("Оберіть принаймні один тип завдання.","error");return}const y=st(a)||"task";x(`Генеруємо ${c.length} тип(и) завдань…`,"loading"),t&&(t.disabled=!0,t.dataset.originalText=t.dataset.originalText||t.textContent,t.textContent="Генерація…"),String(E.category||"").toLowerCase();const b=void 0;try{const u=c.map(g=>Ye(a,g,{items:i,language:"en",seedId:`${y}-${g}`,...l?{additionalInstructions:l}:{}},b).then(v=>({type:g,task:v}))),p=await Promise.allSettled(u),k=p.map((g,v)=>g.status==="fulfilled"?{type:c[v],task:g.value.task}:null).filter(Boolean),D=p.map((g,v)=>g.status==="rejected"?c[v]:null).filter(Boolean);if(!k.length)throw new Error("Не вдалося згенерувати жодного типу завдань");const j=[];k.forEach(({type:g,task:v})=>{const G={...v};G.id||(G.id=`${g}-${y}-${Date.now().toString(36)}${Math.random().toString(36).slice(2,6)}`);const Ae=rt(G);j.push({task:G,key:Ae})}),dt(),window.practice&&typeof window.practice.appendTask=="function"&&(ut(),j.forEach(({task:g,key:v})=>{window.practice.appendTask(g,{key:v})})),U();const de=k.map(({type:g})=>g).join(", ");D.length?x(`Згенеровано: ${de}. Помилки: ${D.join(", ")}`,"success"):x(`Готово! Додано ${k.length} блок(и): ${de}.`,"success")}catch(u){console.error(u);const p=u instanceof Error&&u.message?`Помилка: ${u.message}`:"Не вдалося згенерувати завдання. Перевірте токен або спробуйте ще раз.";x(p,"error")}finally{if(t){const u=t.dataset.originalText||"Згенерувати завдання";t.disabled=!1,t.textContent=u}}}async function pt(){if(!B)return;const e=B.textContent;if(e)try{await navigator.clipboard.writeText(e),x("JSON скопійовано у буфер.","success")}catch(t){console.error(t),x("Не вдалося скопіювати JSON. Спробуйте вручну.","error")}}function ht(e){return`${V(E.id)||V(e.title)||V(E.title)||"practice"||"practice"}.json`}async function ft(){const e=Me(),t=JSON.stringify(e,null,2);if(!t||t==="{}"||t==="[]"){x("Немає даних для завантаження.","error");return}try{const n=new Blob([t],{type:"application/json"}),r=URL.createObjectURL(n),o=document.createElement("a");o.href=r,o.download=ht(e),document.body.appendChild(o),o.click(),document.body.removeChild(o),URL.revokeObjectURL(r),x("Файл завантажено.","success")}catch(n){console.error(n),x("Не вдалося завантажити файл.","error")}}function yt(){if(m){if(at(),m.addEventListener("submit",mt),_&&(_.addEventListener("click",pt),_.disabled=!0),P&&(P.addEventListener("click",ft),P.disabled=!0),S&&M&&W){S.setAttribute("aria-haspopup","true"),S.setAttribute("aria-expanded","false"),S.addEventListener("click",()=>{lt()});const e=n=>{W.contains(n.target)||U()};document.addEventListener("pointerdown",e);const t=n=>{const r=n.relatedTarget;(!r||!W.contains(r))&&U()};W.addEventListener("focusout",t),document.addEventListener("keydown",n=>{n.key==="Escape"&&(U(),S.focus())}),ce().forEach(n=>{n.addEventListener("change",()=>{re()})}),re(),m.addEventListener("submit",()=>{U()})}document.addEventListener("practice:taskRemoved",e=>{const t=e.detail||{};ot(t.key,t.task)}),document.addEventListener("practice:dataLoaded",e=>{const t=e.detail;N.length=0,t&&Array.isArray(t.entries)?(C=t.title??C,I=t.level??I,t.entries.forEach(({task:n,key:r})=>{const o=r||ne(n);N.push({key:o,data:n})})):t&&Array.isArray(t.tasks)?(C=t.title??C,I=t.level??I,t.tasks.forEach(n=>{const r=n&&n.id?String(n.id):ne(n);N.push({key:r,data:n})})):t?(C=t.title??C,I=t.level??I):(C=E.title||C,I=E.level||I),q(),te()}),te(),q(),document.addEventListener("keydown",e=>{e.code&&e.code==="KeyH"&&e.ctrlKey&&e.shiftKey&&(e.preventDefault(),ct())})}}yt();
