...
- Only one option may be different; the rest must relate to "${e}".
      `.trim(),user:h(e,"odd-one-out",r)},audio:{system:`
${o}
OUTPUT SCHEMA:
{
  "id": "audio-${a}-1",
...
REQUIREMENTS:
- Create a coherent listening script (dialogue or monologue) of 120–160 words tied to "${e}".
...
- Generate ${r} comprehension questions in "questions". Mix multiple-choice and short-answer prompts.
...
}`.trim(),user:h(e,"audio",r)},context:{system:`
${o}
OUTPUT SCHEMA:
{
  "id": "context-${a}-1",
...
${l}`}:i}const gt={mcq:10,gap:10,transform:10,match:8,error:10,order:10,short:3,open:4,writing:1,audio:6,roleplay:8,"dialogue-gap":6,"dialogue-order":6,truefalse:8,"definition-match":8,"synonym-clue":8,scramble:8,wordpairs:8,"odd-one-out":8,context:4};function h(e,t,n){return`Topic: ${e}
...
Target vocabulary:
${r}`:n||e||""}function kt(e){if(!Array.isArray(e)||!e.length)return"";const t=r=>{if(!r)return"";const a=r.indexOf(" — "),o=r.indexOf(" – "),s=r.indexOf(" - "),i=[a,o,s].filter(f=>f>=0),l=i.length?Math.min(...i):-1;return(l>=0?r.slice(0,l):r).trim()},n=e.map((r,a)=>{if(!r)return"";const o=r.word||r.term||r.phrase||r.text||`Item ${a+1}`,s=r.translation||r.meaning||r.ua||r.uk||"",i=r.example||r.sentence||r.usage||r.sample||"",l=t(i);let c=`${a+1}. ${o}`;return s&&(c+=` — ${s}`),l&&(c+=` (Example: ${l})`),c}).filter(Boolean);return n.length?n.join(`
`):""}const Ce=document.querySelector(".js-practice-section"),p=document.querySelector("[data-practice-generator]"),V=p==null?void 0:p.querySelector("[data-practice-status]"),F=document.querySelector("[data-generator-output]"),W=document.querySelector("[data-generator-json]"),P=document.querySelector("[data-generator-copy]"),_=document.querySelector("[data-generator-download]"),B=p==null?void 0:p.querySelector("[data-types]"),T=p==null?void 0:p.querySelector("[data-types-trigger]"),A=p==null?void 0:p.querySelector("[data-types-panel]"),$e=document.querySelector("[data-practice-placeholder]"),E=window.lessonContext||{};let $=E.title||"Generated Practice",M=E.level||"custom";const q=[],Q=[];function O(e){e&&e.classList.add("hidden")}function de(e){e&&e.classList.remove("hidden")}function qe(e){return!e||e.classList.contains("hidden")}function me(){return localStorage.getItem("hideGenerateSection")==="true"}function re(){me()?(O(Ce),O(A),O(F),L()):(de(Ce),oe(),N())}function ae(e){return`${e&&typeof e=="object"&&e.id?String(e.id):String((e==null?void 0:e.type)||"task")}-${Date.now()}-${Math.random().toString(16).slice(2,8)}`}function Oe(){return{title:$||E.title||"Generated Practice",level:M||E.level||"custom",tasks:[...q,...Q].map(e=>e.data)}}function N(){if(W){if(me()){O(F);return}if(!q.length&&!Q.length){W.textContent="",O(F),P&&(P.disabled=!0),_&&(_.disabled=!0);return}W.textContent=JSON.stringify(Oe(),null,2),de(F),P&&(P.disabled=!1),_&&(_.disabled=!1)}}function Et(e){const t=ae(e);return Q.push({key:t,data:e}),N(),t}function vt(e,t){const n=a=>{const o=a.findIndex(s=>e&&s.key===e||s.data===t);return o!==-1?(a.splice(o,1),!0):!1};(n(Q)||n(q))&&N()}function Tt(){if(!p)return;const e=p.querySelector('[name="topic"]');if(e&&!e.value){const t=E.title||"Lesson topic";e.value=t}}function v(e,t="idle"){V&&(V.textContent=e,V.dataset.state=t)}function St(e){return String(e||"").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"").slice(0,8)}function X(e){return String(e||"").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"")}function pe(){return Array.from(p.querySelectorAll('input[name="types"]'))}function It(e){const t=e.filter(n=>n.checked).map(n=>{var r,a;return(a=(r=n.nextElementSibling)==null?void 0:r.textContent)==null?void 0:a.trim()}).filter(Boolean);return t.length?t.length<=2?t.join(", "):`${t.slice(0,2).join(", ")} +${t.length-2}`:"Оберіть типи"}function oe(){if(!T)return;const e=pe();T.textContent=It(e)}function L(){!A||!T||qe(A)||(O(A),T.setAttribute("aria-expanded","false"))}function Ct(){if(!A||!T)return;if(qe(A)){de(A),T.setAttribute("aria-expanded","true");const t=A.querySelector('input[name="types"]');t==null||t.focus({preventScroll:!0})}else L()}function $t(){const e=!me();localStorage.setItem("hideGenerateSection",String(e)),re(),N()}function Mt(){const e=document.getElementById("practice"),t=e==null?void 0:e.querySelector("#practice-body");if($e&&$e.remove(),t&&t.children.length===1){const n=t.firstElementChild;n&&/практика поки відсутня/i.test(n.textContent||"")&&n.remove()}}function At(){const e=document.getElementById("practice");if(!e)return;const t=e.querySelector("#practice-body");if(t&&!t.querySelector(".practice-dynamic-title")){const n=document.createElement("div");n.className="practice-dynamic-title",n.innerHTML=`
      <h2>Згенеровані завдання</h2>
      ${E.level?`<p class="muted">Рівень: ${E.level}</p>`:""}
    `,t.prepend(n)}}async function Ut(e){var x,y;if(e.preventDefault(),!p)return;const t=p.querySelector('button[type="submit"]'),n=p.querySelector('[name="topic"]'),r=p.querySelector('[name="count"]'),a=p.querySelector('[name="instructions"]'),o=pe(),s=(x=n==null?void 0:n.value)==null?void 0:x.trim(),i=Number.parseInt((r==null?void 0:r.value)||"10",10)||10,l=((y=a==null?void 0:a.value)==null?void 0:y.trim())||"",c=o.filter(d=>d.checked).map(d=>d.value);if(!s){v("Укажіть тему для генерації завдання.","error"),n==null||n.focus();return}if(!c.length&&typeof L=="function"){v("Оберіть принаймні один тип завдання.","error");return}const f=St(s)||"task";v(`Генеруємо ${c.length} тип(и) завдань…`,"loading"),t&&(t.disabled=!0,t.dataset.originalText=t.dataset.originalText||t.textContent,t.textContent="Генерація…"),String(E.category||"").toLowerCase();const b=void 0;try{const d=c.map(g=>ft(s,g,{items:i,language:"en",seedId:`${f}-${g}`,...l?{additionalInstructions:l}:{}},b).then(C=>({type:g,task:C}))),u=await Promise.allSettled(d),I=u.map((g,C)=>g.status==="fulfilled"?{type:c[C],task:g.value.task}:null).filter(Boolean),z=u.map((g,C)=>g.status==="rejected"?c[C]:null).filter(Boolean);if(!I.length)throw new Error("Не вдалося згенерувати жодного типу завдань");const j=[];I.forEach(({type:g,task:C})=>{const D={...C};D.id||(D.id=`${g}-${f}-${Date.now().toString(36)}${Math.random().toString(36).slice(2,6)}`);const Ne=Et(D);j.push({task:D,key:Ne})}),Mt(),window.practice&&typeof window.practice.appendTask=="function"&&(At(),j.forEach(({task:g,key:C})=>{window.practice.appendTask(g,{key:C})})),L();const ue=I.map(({type:g})=>g).join(", ");z.length?v(`Згенеровано: ${ue}. Помилки: ${z.join(", ")}`,"success"):v(`Готово! Додано ${I.length} блок(и): ${ue}.`,"success")}catch(d){console.error(d);const u=d instanceof Error&&d.message?`Помилка: ${d.message}`:"Не вдалося згенерувати завдання. Перевірте токен або спробуйте ще раз.";v(u,"error")}finally{if(t){const d=t.dataset.originalText||"Згенерувати завдання";t.disabled=!1,t.textContent=d}}}async function Lt(){if(!W)return;const e=W.textContent;if(e)try{await navigator.clipboard.writeText(e),v("JSON скопійовано у буфер.","success")}catch(t){console.error(t),v("Не вдалося скопіювати JSON. Спробуйте вручну.","error")}}function Pt(e){return`${X(E.id)||X(e.title)||X(E.title)||"practice"||"practice"}.json`}async function _t(){const e=Oe(),t=JSON.stringify(e,null,2);if(!t||t==="{}"||t==="[]"){v("Немає даних для завантаження.","error");return}try{const n=new Blob([t],{type:"application/json"}),r=URL.createObjectURL(n),a=document.createElement("a");a.href=r,a.download=Pt(e),document.body.appendChild(a),a.click(),document.body.removeChild(a),URL.revokeObjectURL(r),v("Файл завантажено.","success")}catch(n){console.error(n),v("Не вдалося завантажити файл.","error")}}function qt(){if(p){if(Tt(),p.addEventListener("submit",Ut),P&&(P.addEventListener("click",Lt),P.disabled=!0),_&&(_.addEventListener("click",_t),_.disabled=!0),T&&A&&B){T.setAttribute("aria-haspopup","true"),T.setAttribute("aria-expanded","false"),T.addEventListener("click",()=>{Ct()});const e=n=>{B.contains(n.target)||L()};document.addEventListener("pointerdown",e);const t=n=>{const r=n.relatedTarget;(!r||!B.contains(r))&&L()};B.addEventListener("focusout",t),document.addEventListener("keydown",n=>{n.key==="Escape"&&(L(),T.focus())}),pe().forEach(n=>{n.addEventListener("change",()=>{oe()})}),oe(),p.addEventListener("submit",()=>{L()})}document.addEventListener("practice:taskRemoved",e=>{const t=e.detail||{};vt(t.key,t.task)}),document.addEventListener("practice:dataLoaded",e=>{const t=e.detail;q.length=0,t&&Array.isArray(t.entries)?($=t.title??$,M=t.level??M,t.entries.forEach(({task:n,key:r})=>{const a=r||ae(n);q.push({key:a,data:n})})):t&&Array.isArray(t.tasks)?($=t.title??$,M=t.level??M,t.tasks.forEach(n=>{const r=n&&n.id?String(n.id):ae(n);q.push({key:r,data:n})})):t?($=t.title??$,M=t.level??M):($=E.title||$,M=E.level||M),N(),re()}),re(),N(),document.addEventListener("keydown",e=>{e.code&&e.code==="KeyH"&&e.ctrlKey&&e.shiftKey&&(e.preventDefault(),$t())})}}qt();
